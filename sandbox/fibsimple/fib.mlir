module {
    ...
    // fibloop: int, int ->
    func.func private @fibloop(%s0: !sigi.stack) -> !sigi.stack attributes {sigi.stackType = (i32, i32) -> ()} {
        // -> i, running;
        ...
        %v7 = closure.box [] (%s8 : !sigi.stack) -> !sigi.stack attributes { sigi.stackType = () -> () } { // ->
            closure.return %s8: !sigi.stack
        }
        %s9 = sigi.push %s7, %v7: !closure.box<(!sigi.stack) -> !sigi.stack>
        %v10 = closure.box [%v8_i = %v2_i : i32, %v9_running = %v1_running : i32] (%s10 : !sigi.stack) -> !sigi.stack attributes { sigi.stackType = () -> () } { // ->
            %s11 = sigi.push %s10, %v9_running: i32 // push running
            %s12 = func.call @show(%s11) { sigi.stackType = (i32) -> () } : (!sigi.stack) -> !sigi.stack // int ->
            ...
            %s23 = func.call @fibloop(%s22) { sigi.stackType = (i32, i32) -> () } : (!sigi.stack) -> !sigi.stack // int, int ->
        ...
        %s29 = func.call @apply(%s28) { sigi.stackType = (!closure.box<(!sigi.stack) -> !sigi.stack>) -> () } : (!sigi.stack) -> !sigi.stack // (->) ->
    ...
    func.func @__main__(%s0: !sigi.stack) -> !sigi.stack attributes {sigi.stackType = () -> (), sigi.main} {
        ...
        %s3 = func.call @fibloop(%s2) { sigi.stackType = (i32, i32) -> () } : (!sigi.stack) -> !sigi.stack // int, int ->
