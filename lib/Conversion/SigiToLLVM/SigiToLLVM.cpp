...
struct ConvertSigiPopOpToLLVM : public ConvertOpToLLVMPattern<sigi::PopOp> {
        ...
        ConversionPatternRewriter &rewriter) const override
    {
        ...
        // Error might lie here? op.getElement is not yet converted -> for push
        ...
        if (valueType.isInteger(1)) {
            ...
            return rewriter.notifyMatchFailure(
                op->getLoc(),
                [&](Diagnostic &diag) {
                    diag << "Encountered element with invalid type "
                         << valueType << "during PopOp conversion.";
                });
        }

        LLVM::CallOp pop_call = rewriter.create<LLVM::CallOp>(
            op->getLoc(),
            pop,
            adaptor.getInStack());
...
struct ConvertSigiPushOpToLLVM : public ConvertOpToLLVMPattern<sigi::PushOp> {
        ...
        ConversionPatternRewriter &rewriter) const override
    {
        ...
        if (valueType.isInteger(1)) {
            ...
            // push = getSigiPushClosure(moduleOp,
            // getTypeConverter()->convertType(valueType));
        } else {
            return rewriter.notifyMatchFailure(
                op->getLoc(),
                [&](Diagnostic &diag) {
...
struct convertPrintFuncToFunc
    ...
    LogicalResult matchAndRewrite(
        ...
        ConversionPatternRewriter &rewriter) const override
    {
        if (op.getName() == "sigi::pp") {

            LLVM_DEBUG(llvm::errs() << "Entered rewrite print");
            ModuleOp moduleOp = op->getParentOfType<ModuleOp>();
            MLIRContext* context = op.getContext();
            ...
            LLVM_DEBUG(
                llvm::errs()
                << "Build c signature: " << builtin_signature << "\n");
            rewriter.startOpModification(op);

            if (llvm::failed(op.replaceAllSymbolUses(
                    rewriter.getStringAttr("sigi_builtin__pp"),
                    moduleOp))) {
        ...
        LLVM_DEBUG(
            llvm::errs() << "Op: " << op.getName()
                         << " does not match expected print signature.");
        return llvm::failure();
    };
};
...
void sigiAttributeWalk(auto op, MLIRContext* context)
{
    op->walk([&](LLVM::LLVMFuncOp op) {
        ...
        if (op->hasAttr("sigi.main")) {
            ...
            op->hasAttr("sigi.builtinfunc") && op.getName() == "sigi::pp") {
            LLVM_DEBUG(llvm::errs() << "Found sigi::pp: " << op << "\n");
            LLVM_DEBUG(
                llvm::errs()
                << "Argument Types: " << op.getArgumentTypes()[0] << "\n");
...
void mlir::sigi::populateSigiToLLVMFinalTypeConversions(
    LLVMTypeConverter &typeConverter)
{
    typeConverter.addConversion([&](sigi::StackType stackType) {
